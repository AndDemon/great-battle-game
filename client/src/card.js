export function createCardHTML(card, isPlayable = false, isSelected = false, location = 'board') {
    ...
    const stackBadge = card.stackCount && card.stackCount > 1 
        ? `<div style="position: absolute; top: -10px; right: -10px; background: #e74c3c; color: white; border-radius: 50%; width: 25px; height: 25px; display: flex; align-items: center; justify-content: center; font-weight: bold; border: 2px solid #222; box-shadow: 0 2px 5px rgba(0,0,0,0.5); z-index: 10;">x${card.stackCount}</div>` 
        : '';

    const cardHtml = `
        <div class="${className}" ${dataAttribute} style="position: relative;">
            ${stackBadge} <div class="card-header">
                <span class="card-cost">${card.cost} Energy</span>
                <h4 class="card-title">${card.name}</h4>
            </div>
            ${card.image_url ? `<img class="card-image" src="${card.image_url}" alt="${card.name}" style="width: 100%; height: 60px; object-fit: cover;" />` : ''}
